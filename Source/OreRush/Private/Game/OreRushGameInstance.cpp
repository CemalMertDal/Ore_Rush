...
void UOreRushGameInstance::HandleCreateSessionComplete(FName SessionName, bool bSuccess)
{
	if (SessionInterface.IsValid())
	{
		SessionInterface->ClearOnCreateSessionCompleteDelegate_Handle(CreateHandle);
...
void UOreRushGameInstance::JoinFoundSession(int32 Index)
{
	...
	if (!SessionInterface->JoinSession(0, NAME_GameSession, SearchSettings->SearchResults[Index]))
	{
...
void UOreRushGameInstance::HandleDestroySessionComplete(FName SessionName, bool bSuccess)
{
	if (SessionInterface.IsValid())
	{
		SessionInterface->ClearOnDestroySessionCompleteDelegate_Handle(DestroyHandle);
